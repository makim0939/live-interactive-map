...
const getBoothInViewportByDistance = (booths: BoothInfo[], viewRect: Rect) => {
	...
	const dists = booths.map((booth) => {
		...
		return Math.sqrt(
			(boothCenter.x - viewportCenter.x) ** 2 +
				(boothCenter.y - viewportCenter.y) ** 2,
		);
	});
